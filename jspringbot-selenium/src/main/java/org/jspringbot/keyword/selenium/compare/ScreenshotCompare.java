...
@KeywordInfo(
        ...
        description = "classpath:desc/ScreenshotCompare.txt"
)
public class ScreenshotCompare implements Keyword {
    ...
    public Object execute(Object[] params) throws IOException, InterruptedException {
        if(params.length > 1) {
            helper.compare(String.valueOf(params[0]), Double.parseDouble(String.valueOf(params[1])));
        } else {
            helper.compare(String.valueOf(params[0]));
        }
